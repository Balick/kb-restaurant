import { UilDraggabledots, UilSearch, UilPlus, UilMinus, UilShoppingCartAlt, UilBars  } from '@iconscout/react-unicons'
                ...
                <div className="flex gap-4 mt-[6rem] xl:w-[1032px] flex-col sm2:flex-row  lg:flex-nowrap mx-auto">
                    <div className="h-max flex flex-col justify-center gap-4">
                        <div className="flex flex-col gap-2">
                            ...
                            <p className="sm0:w-[400px] h-[75px] truncate whitespace-normal text-slate-800">Lorem ipsum dolor sit amet,dolores harum labore maiores natus optio pariatur perferendis placeat ratione recusandae repellat, repellendus reprehenderit soluta unde voluptatibus.</p>
                        </div>
                        <CardFood
...
const Button = (props) => {
    ...
    return (
        <button className={"h-[33px] pt-[.1rem] pb-[.5rem] px-5 rounded-[.6rem] "+_clasS}>
            {type}
        </button>
    )
}
...
class CardFood extends Component {
    render() {
        return (
            <div className={"border rounded-[1rem] p-4 h-max " + (this.props.big ? "w-[100%] sm0:w-[400px]" : "w-[235px]")}>
                <div className="flex items-center justify-between relative">
                    ...
                    <div className="border-2 w-[4px] rounded-full flex flex-col items-center leading-[1rem] py-[.1rem] px-[1.5rem] text-sm hover:border-green-500 hover:text-green-500 cursor-pointer font-bold">
                        <small>472</small><small>kcl</small>
                    </div>
                </div>

                <div><img className={"mt-[-19px] mx-auto " + (this.props.big ? null : "h-[143px]")} src={this.props.img} /></div>
                <h2 className={"font-bold " + (this.props.big ? "text-2xl" : "text-center text-md")}>{ this.props.nameFood }</h2>
                <div
                    className={"flex justify-between my-1 " + (this.props.big ? null : "hidden")}
                >
                    <div className="card-food-extra"><small>Fat</small> <g><small>- 26kg</small></g></div>
                    <div className="card-food-extra"><small>Saturates</small> <g><small>- 10kg</small></g></div>
                    <div className="card-food-extra"><small>Sugars</small> <g><small>- 18kg</small></g></div>
                    <div className="card-food-extra"><small>Salt</small> <g><small>- 1kg</small></g></div>
                </div>
                <p className={"text-sm text-slate-600 " + (this.props.big ? null : "h-[2.6rem] w-[100%] truncate whitespace-normal text-center mx-auto")}>
                    {this.props.info}
                </p>

                <div className="mt-4 flex items-center justify-between">
                    <div className="flex gap-[5px] bg-slate-100 w-max p-[.2rem] rounded-[.5rem]">
                        <UilMinus className="cursor-pointer"/> <span>0</span> <UilPlus className="text-green-500 cursor-pointer" />
                    </div>
                    <div className="flex items-center gap-4">
                        ...
                        <button className="bg-green-100 text-green-500 flex items-center gap-[5px] py-1 px-[1rem] rounded-[.5rem]">
                            <UilShoppingCartAlt
                                className={this.props.big ? "border-r border-green-300 pr-1" : "border-0"}
                            />
                            <small
                                className={this.props.big ? null : "hidden"}
                            >Add to Card</small>
                        </button>
                    </div>
                </div>
